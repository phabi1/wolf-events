...
class EventService {
	...
	async item( eventId: string ): Promise< EventDetails > {
		const res = await fetch(
			`/wp-json/wolf-events/v1/events/${ eventId }`
		);
		const data = await res.json();
	...
	async amount( eventId: string ) {
		const res = await fetch(
			`/wp-json/wolf-events/v1/events/${ eventId }/amount`
		);
		const data = await res.json();
		return data.amount;
	}
	...
	private serialize( data: any ) {
		return {
			...
			event_start: data.event_start
				? data.event_start.getTime() / 1000
				: null,
			event_end: data.event_end ? data.event_end.getTime() / 1000 : null,
			registration_start: data.registration_start
				? data.registration_start.getTime() / 1000
				: null,
			registration_end: data.registration_end
				? data.registration_end.getTime() / 1000
				: null,
			sessions: ( data.sessions || [] ).map( ( session: any ) => ( {
				...session,
				session_start: session.session_start
					? session.session_start.getTime() / 1000
					: null,
				session_end: session.session_end
					? session.session_end.getTime() / 1000
					: null,
			} ) ),
		};
	}
	...
	private unserialize( data: any ) {
		return {
			...data,
			event_start: data.event_start
				? new Date( data.event_start * 1000 )
				: null,
			event_end: data.event_end
				? new Date( data.event_end * 1000 )
				: null,
			registration_start: data.registration_start
				? new Date( data.registration_start * 1000 )
				: null,
			registration_end: data.registration_end
				? new Date( data.registration_end * 1000 )
				: null,
			sessions: ( data.sessions || [] ).map( ( session: any ) => ( {
				...session,
				session_start: session.session_start
