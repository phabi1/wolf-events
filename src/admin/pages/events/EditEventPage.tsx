...
export default function EditEventPage() {
		...
		if ( eventId ) {
			...
			events.item( eventId ).then( ( data ) => {
				...
				form.reset( {
					...
					participant_fields: data.participant_fields.map(
						( field: any ) => ( {
							label: field.label,
							type: field.type,
	...
	const onSubmit = async ( model: EventInputs ) => {
		try {
			const data = {
				...model,
				event_start: model.event_start
					? new Date( model.event_start )
					: null,
				event_end: model.event_end ? new Date( model.event_end ) : null,
				registration_start: model.registration_start
					? new Date( model.registration_start )
					: null,
				registration_end: model.registration_end
					? new Date( model.registration_end )
					: null,
			};
